...
External actions and reusable workflows require lowercase full commit SHAs and exact same-line `# vMAJOR.MINOR.PATCH` release comments. Commented workflow examples and listed copyable documentation examples follow that rule. Documentation checks decode YAML keys and values in complete YAML fences and individual fragments, including quoted and flow forms. Each reference needs its own physical annotation line; split scalars and multiple references on one line are rejected. Tagged documentation nodes remain inert and are never constructed as objects. The annotation is descriptive; the `uses:` line remains the action identity. There is no separate version ledger. Offline checks establish syntax, while explicit online verification establishes the release-to-commit correspondence.
