...
- Create, write, replace, and rename workflows **MUST** be symlink-safe regardless of how the target path is classified. The dangerous operation in this class is the rewrite, not the read: [CVE-2026-28684](https://nvd.nist.gov/vuln/detail/CVE-2026-28684) in `python-dotenv` (fixed in 1.2.2) is a local arbitrary-file-overwrite in `set_key()`/`unset_key()` caused by following a symlink while rewriting a `.env` file — a path nearly every project treats as trusted local configuration, and therefore exactly the case a trust-based reading would have exempted. Validate the parent and the intended target against the trusted root before writing, then use the pattern that matches the operation. `O_NOFOLLOW` does not exist on Windows, so probe for it with `hasattr(os, "O_NOFOLLOW")` rather than assuming it is present — and where a workflow in this section depends on no-follow semantics, the flag's absence is a **stop condition** that routes to the platform's equivalent safe primitive or fails the operation, never a silent `getattr(..., 0)` substitution: OR-ing `0` into the flags turns every open into a symlink-following open while the surrounding code continues to claim no-follow behavior.
    ...
    **The normative requirement is the threat model, not the choreography.** Whatever implementation performs this workflow **MUST** defeat at least the following **known threat classes**. The list is **non-exhaustive** — successive review passes kept surfacing further classes, which is itself why the *authoritative* coverage is a tested suite (issue [#844](https://github.com/franklesniak/copilot-repo-template/issues/844)), not this prose; absence of a threat from this list is never permission to ignore it. Each below is a real defect this pattern was refined to close: (1) a symlink or ancestor redirect inserted between a containment check and a later open; (2) time-of-check/time-of-use races across validation, metadata capture, and the final rename; (3) substitution of a temporary name or staging directory that a less-privileged principal can reach; (4) poisoning of the destination's captured metadata by replacing the destination entry before it is read; (5) exposure of secret content during the write window through the file's mode or through inherited ACL entries; (6) failure to reproduce the destination's ownership (UID and GID) on the replacement inode — which is created owned by the writing process, so a `0o640 root:secrets` file rewritten by a helper running as `alice:users` becomes a `0o640 alice:users` inode readable by the wrong group unless ownership is restored — together with loss of POSIX ACLs or security labels across the ownership and mode changes (set-user-ID/set-group-ID bits and file capabilities are handled separately below, because reproducing them is a privilege decision, not preservation); (7) denial of service when the validation open blocks on an attacker-substituted FIFO; and (8) installation of an incompletely written file — a short write, or a failed flush, `close`, or `fsync`, for example an attacker exhausting shared free space or quota to force `ENOSPC` — which the implementation **MUST** prevent by confirming the content is completely and durably persisted **and the destination directory entry synced after the rename** (syncing only the file's data leaves the rename itself losable on a crash), leaving the original destination intact on any failure **before** the atomic rename commits, while a directory-sync failure **after** that commit cannot restore the original and is an indeterminate durability state resolved by the recovery policy in item (10) and the tested suite, not a rollback this prose can promise; (9) a destination inode carrying **additional hard links**, where `os.replace` updates only the selected directory entry while every alias keeps exposing the old inode's contents, so the replacement and all metadata checks can succeed while stale secret content stays reachable through an alias — reject an unexpected `st_nlink`, or define and verify the cross-alias contract explicitly; (10) **leaked artifacts and resource exhaustion** — a temporary file, the staging directory, or held descriptors abandoned when a pre-rename step fails, which an attacker who can repeatedly force `ENOSPC` or a metadata error turns into descriptor, inode, or space exhaustion, and which can strand a temporary file still holding partial secret content — requiring exception-safe cleanup of every artifact on every failure path and a recovery policy for artifacts left by process termination; and (11) **platform-specific security metadata the POSIX items above do not name** — on Windows the replacement inode carries the owner, DACL, SACL, and integrity level of a newly created file rather than the destination's, so a Windows implementation **MUST** reproduce and verify the destination's security descriptor. One attribute class in (6) is governed by privilege rather than preservation: set-user-ID/set-group-ID bits and file capabilities are a privilege *grant*, and the operating system clears them on ordinary modification precisely so attacker-influenced bytes cannot become a privileged executable — so an implementation **MUST** drop them unless the replacement content has authenticated, trusted provenance **and** privilege preservation is explicitly authorized, and **MUST NOT** restore them reflexively. Code **MUST** be verified against this threat model by an adversarial test suite with at least one case per attack *variant*, **not** merely one case per numbered item: several items bundle independently exploitable variants — the distinct races in (2); temporary-name versus staging-directory substitution in (3); the mode and the inherited-ACL mechanisms in (5); each attribute in (6); each leaked-artifact kind plus the repeated-failure case in (10); and each Windows security-descriptor component in (11) — each variant needs its own case, and a missing test **MUST** be treated as a missing defense. Code **MUST NOT** transcribe the illustrative walkthrough linked below into hand-rolled logic and treat that as sufficient: prose cannot be executed, and that walkthrough accumulated defects across more than a dozen review passes precisely because English has no compiler and no test suite. No maintained general-purpose Python library is known to cover this full model — `atomicwrites`, the most maintained option, performs a same-directory temporary write plus `os.replace` with an overwrite guard and does not perform trusted-root descent, metadata preservation, or ACL handling — so "use a library" is not on its own sufficient; the chosen implementation **MUST** be verified against this threat model by the tested suite tracked in issue [#844](https://github.com/franklesniak/copilot-repo-template/issues/844), which — not this prose summary — is the authoritative and maintained record of coverage. A **non-normative illustrative walkthrough** of how a correct implementation reasons about each threat — reference material for building or reviewing a tested implementation, not a specification to copy by hand — is kept out of this instruction file, because syscall-level choreography cannot be validated as prose, and lives in [`docs/python/secure-atomic-replace.md`](https://github.com/franklesniak/copilot-repo-template/blob/HEAD/docs/python/secure-atomic-replace.md).
