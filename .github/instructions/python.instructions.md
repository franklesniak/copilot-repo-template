...
- **[This repo]** Running hook tools directly outside pre-commit, for example `ruff`, `npm run lint:md`, or `yamllint`, **MAY** help during iteration but **MUST NOT** replace the aggregate gate; type checking (`mypy`) and tests (`pytest`) are enforced separately by [`.github/workflows/python-ci.yml`](../workflows/python-ci.yml).
