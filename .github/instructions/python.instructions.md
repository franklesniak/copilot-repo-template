...
- Create, write, replace, and rename workflows **MUST** be symlink-safe regardless of how the target path is classified. The dangerous operation in this class is the rewrite, not the read: [CVE-2026-28684](https://nvd.nist.gov/vuln/detail/CVE-2026-28684) in `python-dotenv` (fixed in 1.2.2) is a local arbitrary-file-overwrite in `set_key()`/`unset_key()` caused by following a symlink while rewriting a `.env` file — a path nearly every project treats as trusted local configuration, and therefore exactly the case a trust-based reading would have exempted. Validate the parent and the intended target against the trusted root before writing, then use the pattern that matches the operation. `O_NOFOLLOW` does not exist on Windows, so read it through `getattr(os, "O_NOFOLLOW", 0)` rather than assuming it is present.
  - **Create-only.** `os.open(path, os.O_WRONLY | os.O_CREAT | os.O_EXCL | getattr(os, "O_NOFOLLOW", 0))` refuses to follow a symlink at `path` and fails if any entry already exists there.
  - **Replacement**, which is what the `.env` case above actually is. The create-only flags **MUST NOT** be adapted for replacement by dropping `O_EXCL`: `O_EXCL` is what makes the open refuse a pre-existing entry, and removing it reintroduces the symlink-following defect the pattern exists to prevent. Instead, anchor the whole operation to a **verified directory handle** rather than re-resolving the parent by pathname. Validating the parent and then passing pathnames to `os.open()` and `os.replace()` still leaves a TOCTOU window: a party who can write the parent's ancestry can swap that directory for a symlink between the check and the calls, and `O_NOFOLLOW` guards only the final component, so both operations can land outside the trusted root. Open the validated parent once with `os.open(parent, os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0))`, create the temporary file with the create-only flags and `dir_fd=` that handle, write the new contents, then move it into place with `os.replace(tmp_name, dest_name, src_dir_fd=dir_fd, dst_dir_fd=dir_fd)` — both names resolved relative to the handle, never re-walked. Reject a symlinked destination before replacing (`Path.is_symlink()`, or `Path.is_file(follow_symlinks=False)` on Python 3.13+). Probe support with `os.open in os.supports_dir_fd` and **`os.rename in os.supports_dir_fd`**: `os.replace` is *not* a member of that set even on platforms where its `src_dir_fd`/`dst_dir_fd` parameters work, because it shares the underlying `renameat` with `os.rename`, so probing on `os.replace` itself reports a false negative. Directory-handle operations are POSIX-only; where they are unavailable, keep the containment checks and record the residual race rather than assuming the pathname form is equivalent.
