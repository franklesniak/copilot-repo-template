...
- **[All]** When code annotates a parameter that accepts either `int` or `float`, the annotation **SHOULD** be `float`: the typing specification's numeric tower shortcut lets a `float` parameter accept `int` arguments, and Ruff's PYI041 reports `int | float` in parameter annotations and rewrites it to `float` when fixes are applied. That shortcut applies to arguments only, so when such a function can return either type, its return annotation **SHOULD** stay `int | float`, for example `def add(a: float, b: float) -> int | float`; the union keeps the precise result type visible to callers and type checkers, which then reject `float`-only calls such as `.hex()` on a result that can be an `int`. The typing best-practices guide recommends `float` over `int | float` as a general style; this guide applies that recommendation to parameters, where the union is redundant, and keeps the union on return annotations, where it is informative. Code **SHOULD** coerce the result to `float` only when the contract is a `float` result.
