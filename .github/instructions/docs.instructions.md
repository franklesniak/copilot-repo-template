...
- **Rules.**
  ...
  - When a shell-command example depends on a third-party CLI tool that is not part of standard installations on the target environments listed in the **Scope.** paragraph (for example, `rg`/ripgrep, `fd`/`fdfind`, `jq`, `yq`, `eza`/`exa`, `bat`, `delta`, or similar), the example MUST either document a portable fallback that uses tools available without extra installs (for example, `grep -E -R`, `find`, `awk`, `sort`, or `uniq`) or tools already established as prerequisites in the surrounding document (for example, `git grep -E` when `git` is already a documented prerequisite, since `git` itself is not a default install on every target environment listed in the **Scope.** paragraph), or explicitly label the tool dependency in surrounding prose so readers know to install it before running the example. Examples in downstream-facing guides, such as `TEMPLATE_UPDATE_PROCEDURE.md`, `GETTING_STARTED_*.md`, `OPTIONAL_CONFIGURATIONS.md`, and onboarding READMEs, SHOULD prefer the portable form by default because adopters may be on any platform without prior tool installation.
