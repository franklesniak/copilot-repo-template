...
For this template's host modules, documentation **MUST** keep GitHub as the primary/default platform and describe Azure DevOps Services as optional additive module support. When retained protected or shared docs refer to Azure DevOps Services setup, validation, security scanning, dependency-update choices, or organization URL forms, prefer the durable Azure DevOps Services support guide at `docs/azure-devops-support.md` when that guide is retained. If the reference is a Markdown link from a file that can be retained without the Azure guide, guard it with the registered `azure-devops-guide-reference-only` marker or use non-link path prose instead.
