...
For this template's host modules, documentation **MUST** keep GitHub as the primary/default platform and describe Azure DevOps Services as optional additive module support. That requirement states, for documentation, the canonical [Host-Specific PR Review Protocols](../copilot-instructions.md#host-specific-pr-review-protocols) section; it is an interpretation of that section and creates no exception to it. When retained protected or shared docs refer to Azure DevOps Services setup, validation, security scanning, dependency-update choices, or organization URL forms, prefer the durable Azure DevOps Services support guide when that guide is retained. If the reference is a Markdown link from a file that can be retained without the Azure guide, guard it with the registered `azure-devops-guide-reference-only` marker or use non-link path prose instead.
