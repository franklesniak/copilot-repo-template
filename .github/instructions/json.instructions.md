...
The requirements below state how the canonical [Determinism and Correctness Rules](../copilot-instructions.md#determinism-and-correctness-rules) apply to generated JSON. The canonical "Preserve formatting, indentation, and ordering when processing structured content" directive governs content a tool is transforming, where reordering churns a diff the tool does not own. A generator emits an artifact it owns and rewrites wholesale, so there is no prior order to preserve; the byte-identical reproducibility requirement below is what makes those diffs meaningful instead. Where order carries meaning the directive does bind, and the bullets below require exactly that — the generator preserves input order wherever ordering is semantic. These are interpretations of the canonical rules and create no exception to them.
