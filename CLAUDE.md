...
1. **Request both reviews.** First, record both detection baselines and the request-time PR head SHA (these **MUST** be recorded before requesting either review):
    ...
    - Record the corresponding newest review, inline-comment, PR-conversation, and complete `reviewThreads` evidence for `chatgpt-codex-connector[bot]` (or note that no such Codex event exists yet). Preserve each source's ID and timestamp as the Codex baseline. The exact `@codex review` trigger is not a completed review result, and a generic running-status summary is not terminal evidence.
...
2. **Wait for the review (active polling).** Immediately after requesting the review, begin an active poll loop — do **not** rely solely on webhook delivery, which may be delayed or never arrive. The poll loop **MUST** follow these rules:
    ...
    - **Detection criterion.** On each poll, use **both** `get_reviews` **and** `get_review_comments` as co-equal detection signals for Copilot, and query the complete Codex review, review-comment, PR-conversation, and `reviewThreads` surfaces for `chatgpt-codex-connector[bot]`. A new Copilot event is detected when **either** of the following is true:
      ...
      If no baseline exists (no prior review by the bot), any review or comment by the bot is considered new. A fresh set of Copilot comments newer than the baseline is evidence that Copilot has arrived, but the agent **MUST NOT** proceed to step 3 until remote Codex also has an attributable terminal result for the same head.
    - **Remote Codex arrival.** Codex is detected only when a post-baseline Codex-authored review submission, inline review comment, or completed PR-conversation result is attributable to the exact request and the request-time head SHA. A result on another head is stale. The poll MUST paginate every Codex source to completion. Missing, pending, running, failed, canceled, skipped, expired, timed-out, and ambiguous Codex results are not clean; if Codex has not reached a terminal result, the loop MUST continue polling or pause under the bounded failure rules. The loop MUST NOT advance to coverage or findings merely because Copilot arrived first, and it MUST wait for terminal results from both reviewers.
    - **Co-equal detection preserved.** When one detection source (`get_reviews` or `get_review_comments`, or equivalent authenticated sources) returns a successful new-event signal and the other source fails or is indeterminate, the agent **MAY** record the new event for that reviewer, but **MUST NOT** proceed to step 3 or declare clean until both reviewers have attributable terminal results. By contrast, when one source returns a successful no-new-event signal and the other source fails or is indeterminate, the cycle is **indeterminate**: the agent **MUST** treat it as a failed cycle (not a confirmed-successful no-review poll), **MUST NOT** advance the 10-poll timeout counter under **Failed cycles do not consume the timeout** below, and **MUST** apply the retry-or-pause behavior under **Retry or pause on polling failure** below. A cycle qualifies as a **confirmed-successful no-review poll** only when **both** detection sources were queried, parsed, and pagination-completeness verified successfully and **both** returned no new events. The co-equal detection-source contract remains unchanged; only the failure-handling semantics are clarified.
    ...
    - **Remote Codex timeout.** Maintain a missing-Codex counter independently from the combined no-review counter. If Codex has not produced an attributable terminal result after **10 confirmed-successful missing-Codex polls** (at least 10 minutes wall-clock after Copilot arrives), **PAUSE** the loop and name Codex as the missing reviewer. A missing-Codex poll counts only when the Codex sources are queried, parsed, and pagination-complete with no terminal Codex result; failed or indeterminate Codex observations do not advance the counter. Do not declare clean, re-request, or advance to a new round until the owner resumes the loop or the reviewer result arrives.
    - **Remote Codex terminal failure.** If Codex produces a terminal `failed`, `canceled`, `skipped`, `expired`, or `timed-out` result, **PAUSE** the loop with a visible message such as `Review loop paused: remote Codex returned terminal status <status> for head <sha>.` Do not treat that result as missing, clean, or an automatic retry. Name the status and head, preserve the evidence, and require owner resumption with a new baseline before requesting another round.
    ...
    - **On success.** Record each review when it arrives. Proceed to step 3 only after both Copilot and remote Codex have attributable terminal results for the same request-time head.
