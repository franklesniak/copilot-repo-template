...
1. **Signal processing (conditional).** If the available tooling supports **both** adding and removing emoji reactions on review comments, add an `eyes` (👀) reaction when you begin processing a comment and remove it when you finish, whether that occurs after step 9 or via the early-exit path in step 2. This signals to the reviewer that the feedback is being actively worked on. Both halves are required: a marker you cannot clear leaves the comment flagged as in progress forever, which misinforms the reviewer more than adding no marker at all. If either capability is missing, skip this step silently and process the comment normally. Determine the capability from the tooling actually available in the current runtime rather than from any capability claim recorded here, since tool surfaces change without notice. *(Informational, not operative: at the time of this revision the GitHub MCP server can create reactions on review comments — `add_reply_to_pull_request_comment` accepts a `reaction` value — but exposes no removal endpoint, so the condition above resolves to skip on that runtime.)*
...
9. **Resolve or leave open.** If **no** style guide update was recommended in step 8, resolve the review comment thread when tooling permits — for example the `resolve_review_thread` tool, which takes the GraphQL thread node ID (`PRRT_…`) that `get_review_comments` returns on each thread, an equivalent `resolveReviewThread` GraphQL mutation, or manual owner action. If a style guide update **was** recommended, leave the thread **open** so the owner can see and act on the prompt before it is dismissed. If no thread-resolution capability is available in the current runtime, recording in the reply that the thread needs manual resolution **is** the step 9 completion work for that case, and it satisfies the Automated Review Loop step 5 requirement to handle that work before the comment counts as fully processed; make that record, then continue rather than stalling on an operation the runtime cannot perform. As in step 1, establish the capability from the tooling actually available rather than from any capability claim recorded here.
