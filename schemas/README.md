...
Catalog `heading` and string `next_heading` values must be complete single-line ATX headings without trailing ASCII space or tab. The schema and semantic loader reject invalid spelling before document validation, including trailing CR or LF. Internal whitespace and Unicode remain literal; `null` still denotes an EOF successor.

Within one file contract, distinct sections cannot share an immediate successor, including the EOF successor. The semantic loader rejects these unsatisfiable catalogs before document validation. Partial chains and boundaries outside the contracted section set remain supported; each declared successor must have at most one predecessor.

The optional `required_block_order` lists `paragraph` and `table` tokens in their complete interleaved order. Omission means all required paragraphs followed by all required tables; it does not disable order checking. Explicit token counts must match the corresponding inventories. Existing missing-clause and table checks remain separate, while inversions among recognized blocks produce a content-bound ordering failure. Paragraph-inventory and table mismatch waivers include expected placement and the observed section inventory, so a waiver cannot silently authorize moving altered prose or a changed or malformed table.

Space-based hard line breaks inside operative paragraphs or list items are unsupported and produce a separate source-bound failure. The scanner retains two-or-more trailing spaces until block boundaries are known; ordinary wrapping and block-final padding still normalize. Recognized thematic-break lines retain their existing source representation and do not create hard-break failures for padding immediately before or on the break. Fenced/quoted examples and active inline-code spans keep their existing precedence. Backslash breaks remain literal source. This bounded check does not implement general inline HTML parsing.

Indented lazy continuations across omitted, partial, or newly deeper quote prefixes in a multilevel quote are unsupported. The scanner retains the original ambiguous region so a following live clause cannot disappear. Ordinary single-level indented paragraph continuations and non-indented nested lazy text remain supported. This conservative boundary does not claim full container parsing; use explicit unambiguous examples when authoring governed sections.
...
Complete valid inline comment tokens remain in governed paragraph and list-item source before the existing paragraph whitespace normalization. The scanner does not delete those tokens or insert replacement spaces. For example, `See [policy]<!-- note -->(docs/policy.md).` cannot satisfy `See [policy](docs/policy.md).`: deleting the comment would construct a link absent from the source. Inserting even a renderer-neutral inline comment causes drift against a comment-free clause. Put maintenance notes in standalone comment blocks, or explicitly contract the same supported inline spelling. Distinct comment content changes the failure identity when it remains distinct after normalization; ASCII whitespace-only changes can still normalize equally. Quoted list content with more than four padding columns starts item code and cannot establish lazy paragraph continuation. Tabs in quoted container or list structural prefixes are unsupported and remain visible in failure inventories; tabs after ordinary text begins remain supported. Comment removal cannot create a heading or list marker absent from the source. Comment-bearing table rows and nonblank HTML-comment-block closing tails are unsupported original-text inventory. Distinct tables can share column headers; duplicate table identity includes the complete headers and ordered rows, and the document must retain the full ordered table inventory.
...
The validator indexes operative line positions and next headings once per document. Catalog cycle checks and optional-successor traversal reuse completed paths within each call, and paragraph matching uses indexed positions. These checks avoid repeated whole-document or shared-chain scans for ordinary valid catalogs. Invalid overlapping regions still require their actual content for stable waiver diagnostics; this is not a claim of linear work for every malformed input or output size.
