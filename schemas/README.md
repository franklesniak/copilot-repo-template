...
Contract table cells must use canonical nonempty spelling: non-whitespace words separated by one ASCII space, without leading/trailing whitespace or pipes. The schema and semantic loader use an explicit shared whitespace grammar to give Python and ECMAScript consumers the same result, including control separators and U+FEFF; they reject invalid cells before protected-file drift checks. Unicode text and Markdown markup remain supported. Inline-code delimiter runs are indexed once within the supported block boundaries; unmatched runs stay literal without repeated paragraph-tail searches.
