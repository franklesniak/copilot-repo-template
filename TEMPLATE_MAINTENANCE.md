...
1. Visit the provider registries:
   ...
   > **Note:** Terraform Registry navigation links in Terraform-file comments **MUST** use the `latest` path segment, not a pinned provider or module version. The same `/latest/` requirement applies to Terraform Registry navigation links in instructional Markdown under this template (such as the provider links above); the Markdown half is binding via the ADR until it is mirrored into an `applyTo`-scoped Markdown instruction file. Within Terraform-file comments the canonical, agent-loadable rule lives in [`.github/instructions/terraform.instructions.md`](.github/instructions/terraform.instructions.md), which is scoped to Terraform extensions via `applyTo`. The authoritative version sources are `required_providers` blocks (`terraform.tf` / `versions.tf`) and `.terraform.lock.hcl` for *providers*, and module `source` and `version` arguments for *modules* (`.terraform.lock.hcl` does not record module versions). Pinned Registry URLs in comments drift silently because version updates — whether by maintainers or by future dependency automation (this template does not currently configure Terraform-ecosystem updates in [`.github/dependabot.yml`](.github/dependabot.yml)) — touch version constraints, module `version` arguments, and `.terraform.lock.hcl` (for providers), but do not rewrite free-text URLs embedded in comments. See the **Terraform Registry Reference URLs Use /latest/** ADR in [`.github/TEMPLATE_DESIGN_DECISIONS.md`](.github/TEMPLATE_DESIGN_DECISIONS.md) for the rationale; the normative rule for Terraform-file comments lives in [`.github/instructions/terraform.instructions.md`](.github/instructions/terraform.instructions.md).
