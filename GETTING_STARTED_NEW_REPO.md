...
The default adoption mode for protected files and template-derived governance, community, process, workflow, and collaboration files is `minimal-preservation`: keep upstream wording and structure, substitute placeholders, trim sections owned by unadopted manifest modules, fix broken links, and record required local overrides in `.template-sync/marker.yml` when template sync support is retained. Select `tailored` only when the maintainer explicitly wants broader downstream rewriting for a specific file or file set. Record that choice before editing so agents do not repeatedly prompt when the default applies. An adoption mode bounds how much of a protected file an authorized edit may change; it is not itself authorization to change one. The canonical **Protected Instruction Files** rule in [the canonical repository instructions](https://github.com/franklesniak/copilot-repo-template/blob/HEAD/.github/copilot-instructions.md) still requires that "the repository owner or maintainer has directly and explicitly authorized the specific instruction-file change in the current task", and states that "Implied consent is insufficient." This paragraph is an interpretation of that rule and creates no exception to it.
...
Deleting a file under `.github/instructions/` is a protected instruction-file change, exactly like editing one. The canonical **Protected Instruction Files** rule in [the canonical repository instructions](https://github.com/franklesniak/copilot-repo-template/blob/HEAD/.github/copilot-instructions.md) states that agents "**MUST NOT** create, edit, delete, rename, or otherwise change protected instruction files unless the repository owner or maintainer has directly and explicitly authorized the specific instruction-file change in the current task", so the items below that delete a `.github/instructions/*.instructions.md` guide carry the same authorization requirement as the reference-removal items that name it explicitly. Where this checklist states how those canonical rules apply during stack selection, it is an interpretation of them and creates no exception to them.
