...
Deleting a file under `.github/instructions/` is a protected instruction-file change, exactly like editing one. The canonical **Protected Instruction Files** rule in [`.github/copilot-instructions.md`](.github/copilot-instructions.md) states that agents "**MUST NOT** create, edit, delete, rename, or otherwise change protected instruction files unless the repository owner or maintainer has directly and explicitly authorized the specific instruction-file change in the current task", so the "Delete `.github/instructions/*.instructions.md`" items below carry the same authorization requirement as the reference-removal items that name it explicitly. Where this checklist states how those canonical rules apply during stack selection, it is an interpretation of them and creates no exception to them.
