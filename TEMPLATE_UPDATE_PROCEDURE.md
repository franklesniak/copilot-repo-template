...
| PR-ready branch | The sync is ready for normal review **and every required validation check is passing**. | Commit the intended changes, record validation results, and prepare the sync summary for the PR body. Opening the PR MAY happen immediately when remote tooling and owner direction allow it. This mode is unavailable while any required validation check is failing, whatever the owner has deferred; finalize through a local handoff mode above and leave the push to the owner. |
