...
The repository-wide commit-hygiene rule in [`.github/copilot-instructions.md`](.github/copilot-instructions.md) forbids standalone formatting-only or lint-only commits: under **What Not to Do** it bars separate "fix formatting" or "fix linting" commits and directs authors to "include all auto-fixes in the same commit as your changes", and **Pre-commit Discipline (CRITICAL)** step 3 directs the agent to amend or fold in fixes "rather than landing a standalone formatting-only or lint-only commit". A `.gitattributes` re-normalization pass satisfies that rule directly, and needs no exception to it, provided the commit carries the change that caused the churn.

Git's own [`gitattributes` documentation](https://git-scm.com/docs/gitattributes) shows the shape. Its renormalization recipe writes `.gitattributes`, runs `git add --renormalize .`, and finishes with `git commit -m "Introduce end-of-line normalization"` — and because `git add --renormalize .` stages everything under `.`, that single commit contains **both** the `.gitattributes` change and the renormalized files. Git separates the normalization from *other work*, not from *its own cause*.
...
`deferred follow-up` is a classification and disclosure disposition. It records that a failure exists, who owns it, and that the sync did not cause it. It is **not** push authorization, and it does not make a failing required check compliant. An agent **MUST NOT** push on its own authority while a required validation check is failing, whatever the failure's origin: fix the failure, or obtain the repository owner's explicit decision to accept the pre-existing debt and push in that state. Only the repository owner can accept that debt for their own repository; agent judgment that a failure is "pre-existing" does not substitute for that decision. Where the two readings of the canonical prohibition diverge, the canonical rule controls and the failure is fixed before pushing.
