...
The repository-wide commit-hygiene rule in [`.github/copilot-instructions.md`](.github/copilot-instructions.md) still forbids routine standalone formatting-only or lint-only commits. That rule, under **What Not to Do**, bars separate "fix formatting" or "fix linting" commits and directs authors to "include all auto-fixes in the same commit as your changes"; **Pre-commit Discipline (CRITICAL)** step 3 directs the agent to amend or fold in fixes "rather than landing a standalone formatting-only or lint-only commit". Both target auto-fixes produced alongside a substantive change; a `.gitattributes` re-normalization pass is a reviewed content change caused by a configuration change, not an auto-fix accompanying one. Git's own [`gitattributes` documentation](https://git-scm.com/docs/gitattributes) prescribes that separation for this operation, ending its renormalization recipe with `git commit -m "Introduce end-of-line normalization"` as a commit of its own. The narrower disposition below rests on that scope reading; it is an interpretation of those canonical rules and creates no exception to them. Git's recipe is cited as evidence that this operation is conventionally its own commit, not as authority over repository policy: if the canonical rule is read to reach a `.gitattributes` re-normalization pass, the canonical rule controls and the normalization MUST be folded into the substantive sync commit. A separate normalization commit is allowed in this procedure when `.gitattributes` adoption causes broad mechanical line-ending churn and separating that churn materially improves reviewability. That commit MUST contain only the reviewed normalization effects caused by the `.gitattributes` change, MUST stay adjacent to the related sync change, MUST be recorded in the sync summary, and MUST state in its message that it is a `.gitattributes` re-normalization pass, so the basis for separating it is auditable from the history rather than inferred from the diff. If the normalization is small, include it in the same substantive sync commit instead.
