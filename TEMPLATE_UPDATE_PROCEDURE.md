...
`deferred follow-up` is a classification and disclosure disposition. It records that a failure exists, who owns it, and that the sync did not cause it. It is **not** push authorization, and it does not make a failing required check compliant. The canonical prohibition is scoped to the code being pushed, not to the moment of pushing. This section applies that scoping to every required validation check as its own requirement, restating canonical for pre-commit CI and going beyond it for other required checks: an agent **MUST NOT** push a change that leaves any required validation check failing, whatever the failure's origin. Pushing the commit that makes a failing check pass is not what this rule forbids; it is the canonical remediation the rule points to, which directs the agent to "fix issues and re-run until all checks pass". Fix the failure and re-run until every required check passes, or stop and hand the decision to the repository owner. A partial fix that leaves any required check still failing is a prohibited push, not progress.
