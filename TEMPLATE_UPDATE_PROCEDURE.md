...
The repository-wide commit-hygiene rule in [the canonical repository instructions](https://github.com/franklesniak/copilot-repo-template/blob/HEAD/.github/copilot-instructions.md) forbids standalone formatting-only or lint-only commits: under **What Not to Do** it bars separate "fix formatting" or "fix linting" commits and directs authors to "include all auto-fixes in the same commit as your changes", and **Pre-commit Discipline (CRITICAL)** step 3 directs the agent to amend or fold in fixes "rather than landing a standalone formatting-only or lint-only commit". A `.gitattributes` re-normalization pass satisfies that rule directly, and needs no exception to it, provided the commit carries the change that caused the churn.

Git's own [`gitattributes` documentation](https://git-scm.com/docs/gitattributes) shows the shape. Its renormalization recipe writes `.gitattributes`, runs `git add --renormalize .`, and finishes with `git commit -m "Introduce end-of-line normalization"`, keeping the normalization in one commit separate from *other work*. Note that `--renormalize` implies `-u` and so stages **tracked files only** (`git add -h`: "renormalize EOL of tracked files"). When `.gitattributes` is new or reintroduced it is untracked, so the recipe as written leaves it out of that commit. Stage it explicitly — `git add .gitattributes` before or after the renormalization — so the commit carries the change that caused the churn.

So the normalization commit **MUST** contain the `.gitattributes` change together with the reviewed renormalization effects that change produced, and nothing else. Committed that way it is a substantive configuration change carrying its own mechanical churn, not a formatting-only commit, and the canonical rule is met as written. It **MUST** stay adjacent to the related sync change, **MUST** be recorded in the sync summary, and its message **MUST** identify it as a `.gitattributes` re-normalization pass so the commit's shape is auditable from history rather than inferred from the diff. Splitting the `.gitattributes` change into one commit and its renormalization effects into another is what the canonical rule prohibits, and **MUST NOT** be done. If the churn is small enough that separating it does not materially improve reviewability, include the `.gitattributes` change and its effects in the same substantive sync commit instead.
...
`deferred follow-up` is a classification and disclosure disposition. It records that a failure exists, who owns it, and that the sync did not cause it. It is **not** push authorization, and it does not make a failing required check compliant. The canonical prohibition is scoped to the code being pushed: what it forbids is pushing a change that will still fail, not pushing at a moment when a check happens to be red. This section applies that scoping to every required validation check as its own requirement, restating canonical for pre-commit CI and going beyond it for other required checks: an agent **MUST NOT** push a change that would still leave a required validation check failing once that check re-runs against the pushed head, whatever the failure's origin. Pushing the commit that makes a failing check pass is not what this rule forbids; it is the canonical remediation the rule points to, which directs the agent to "fix issues and re-run until all checks pass". Fix the failure and re-run until every required check passes, or stop and hand the decision to the repository owner. A partial fix that leaves any required check still failing is a prohibited push, not progress.

The rule binds on what the agent can establish before pushing. A required check that cannot produce a result until the commit is reachable remotely — an Azure Pipelines queued run or a branch-policy build validation, for example — has no pre-push outcome to leave failing, so it cannot be evaluated under the sentence above. For those checks the obligation is: run every check that *can* run beforehand and have them all pass, push, then treat the service's result as a required check like any other. If it comes back failing, that failure is now the agent's to fix or to hand to the repository owner, and the next push is governed by the same rule. This is not a license to push with a locally runnable check still red on the grounds that some remote check has to run anyway; the distinction is whether a check *can* run before the push, never whether it happened to be run.

Obtaining the owner's approval does not convert an agent push of failing code into a permitted one: for pre-commit CI the canonical prohibition is absolute — "**NEVER** push code that will fail pre-commit CI" — and states no owner-authorization exception, and an actual exception would have to be created in `.github/copilot-instructions.md` rather than here. Accepting pre-existing debt and pushing in the failing state is the owner's call to make and to act on; it is never the agent's, and no classification in the table below converts it into one. Only the repository owner can accept that debt for their own repository; agent judgment that a failure is "pre-existing" does not substitute for that decision.
...
| deferred follow-up | The failure is pre-existing downstream debt or a policy decision that is outside the sync's approved scope. | Leave the validator failure visible, record the owner decision or follow-up needed, and do not describe the sync as PR-ready while any required validation check is failing. Owner deferral records who owns the debt; it does not confer PR readiness, and the agent finalizes through a local handoff mode instead. |
...
| PR-ready branch | The sync is ready for normal review **and every required validation check is passing**. | Commit the intended changes, record validation results, and prepare the sync summary for the PR body. Opening the PR MAY happen immediately when remote tooling and owner direction allow it. This mode is unavailable while any required validation check is failing, whatever the owner has deferred; finalize through a local handoff mode above. That gate governs PR readiness and opening the PR, not remediation. Where a required check can only produce a result once the commit is reachable remotely, the agent MAY push a remediation candidate after every check that can run beforehand passes, exactly as the Validation Triage rule provides — otherwise the failure could never be fixed, because the fix needs the push that produces the next result. What stays with the owner is declaring the branch PR-ready, or opening the PR, while a required check is failing. |
