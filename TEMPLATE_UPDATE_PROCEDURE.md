...
The rule binds on what the agent can establish before pushing. A required check that cannot produce a result until the commit is reachable remotely — an Azure Pipelines queued run or a branch-policy build validation, for example — has no pre-push outcome to leave failing, so it cannot be evaluated under the sentence above. For those checks the obligation is: run every check that *can* run beforehand and have them all pass, push, then treat the service's result as a required check like any other. If it comes back failing, that failure is now the agent's to fix or to hand to the repository owner, and the next push is governed by the same rule. This is not a licence to push with a locally runnable check still red on the grounds that some remote check has to run anyway; the distinction is whether a check *can* run before the push, never whether it happened to be run.
