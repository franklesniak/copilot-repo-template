...
`deferred follow-up` is a classification and disclosure disposition. It records that a failure exists, who owns it, and that the sync did not cause it. It is **not** push authorization, and it does not make a failing required check compliant. An agent **MUST NOT** push on its own authority while a required validation check is failing, whatever the failure's origin: fix the failure, or stop and obtain the repository owner's explicit decision. Accepting pre-existing debt and pushing in the failing state is the owner's call to make and to act on; it is never the agent's, and no classification in the table below converts it into one. Only the repository owner can accept that debt for their own repository; agent judgment that a failure is "pre-existing" does not substitute for that decision.
