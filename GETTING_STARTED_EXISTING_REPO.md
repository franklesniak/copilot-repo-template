...
The helper copies only retained manifest-owned files, removes inline blocks for excluded modules, optionally reuses the approved placeholder replacement helper against a staging tree, and refuses to overwrite non-identical existing target files unless a path-scoped decision authorizes or records the conflict. Use `--template-root` for an existing source checkout, or use `--template-ref REF` / `--template-revision FULL_SHA` with `--template-repo PATH` to let the helper create and remove a private detached source worktree from locally available Git objects. The helper does not fetch, pull, merge, or rebase.

For `--template-root`, the summary can report an observed source commit when the local root is a complete, clean Git worktree at its top level. That reported commit is source-checkout identity only; it is not automatically reviewed state and is not written to `template_sync.last_reviewed_template_commit` by default. After completing adoption review, rerun with `--stamp-resolved-source-as-reviewed`, or pass `--last-reviewed-template-commit FULL_SHA`. The stamp flag is an operator assertion that review is complete and that the clean local source commit is the intended upstream lineage; a clean local `HEAD` proves local checkout state, not lineage by itself.

Local-root stampability covers tracked content and rejects dirty tracked files plus non-ignored untracked files; ignored untracked files are outside the stampability gate. A `--template-root` inside a subdirectory of a larger Git worktree is not stampable and is not promoted to the worktree root. Sparse checkout, skip-worktree, and assume-unchanged states are not stampable because tracked files can be absent or hidden while ordinary status remains clean. On Git older than 2.36, or when Git version parsing fails, configured `core.fsmonitor` also makes a local root not stampable; use a Git executable detected as 2.36 or newer, or pass `--last-reviewed-template-commit FULL_SHA` after review. Template sources that record submodule gitlinks are not stampable because the materializer neither initializes nor verifies submodules. Restore a full template source checkout before materializing from it when any of these conditions apply.

For `--template-ref` and `--template-revision`, the helper reports the supplied source ref or revision, resolved source SHA, source repository, diagnostic temporary checkout path, cleanup status, and target root, then rejects a reviewed SHA that differs from the resolved source SHA. Do not copy any materialized source SHA into `template_sync.last_reviewed_template_commit` until adoption review is complete. In a non-empty repository, conflicts for existing files such as `README.md`, `LICENSE`, and `.gitignore` are expected first-adoption decisions. Resolve them per path through `template_sync.local_overrides`, `template_sync.protected_file_decisions`, or `template_sync.deferred_protected_candidates`; they are not runtime/tool failures.
...
The template treats `.github/copilot-instructions.md`, `.github/instructions/**`, `.cursor/rules/**`, and root agent instruction files such as `.hermes.md`, `AGENTS.md`, `CLAUDE.md`, and `GEMINI.md` as protected governance files. This step applies the canonical [**Protected Instruction Files**](https://github.com/franklesniak/copilot-repo-template/blob/HEAD/.github/copilot-instructions.md#protected-instruction-files) rule and its [**Template Adoption and Stack Selection**](https://github.com/franklesniak/copilot-repo-template/blob/HEAD/.github/copilot-instructions.md#template-adoption-and-stack-selection) subsection, which require that "the repository owner or maintainer has directly and explicitly authorized the specific instruction-file change in the current task", state that "Implied consent is insufficient", and bar inferring authorization from "Pre-commit, formatting, linting, validation, or other cleanup work." When adopting into an existing repository:
...
If mutating hooks or fixers should intentionally update files, run fix mode, inspect the changed-file summary, keep or discard final Git status changes intentionally, then rerun check mode:
...
Expected result: `--plan-only` prints the Git-visible file collection result, explanatory notes, and the deterministic numbered command plan without running validation commands; planned commands printed in this mode are not executed validation-command boundaries, so status changes in plan-only mode cannot receive command-boundary attribution. The normal check run prints the same command plan before the first validation command starts, warns that cold pre-commit hook environment bootstrapping may be slow, then validates tracked plus untracked non-ignored regular files. It differs from `pre-commit run --all-files` because it builds its file list from `git ls-files --cached --others --exclude-standard`, then runs `pre-commit run --files ...` against that list so newly copied adoption files are checked before they are committed. Each planned command reports its group label, index, UTC start and end timestamps, elapsed time, and exit status; the run also reports a deterministic before/after Git changed-file summary, supplements it with command-boundary status transitions observed after executed planned validation commands when transitions occur, reports total elapsed time, and continues through the full plan so multiple failures are reported together. Nonzero planned commands still fail the run. In check mode, when planned commands otherwise pass, a final aggregate Git status change exits with the distinct changed-file code after status-scoped follow-up guidance. In fix mode, final aggregate Git status changes are tolerated only when planned commands otherwise pass. Transient command-boundary status transitions are reported but do not change the exit code by themselves. If a planned command also fails, resolve the validation failure separately from any changed-file keep/revert decision. If the `pre-commit` console script is not on PATH, it uses the equivalent `python -m pre_commit run --files ...` form. When the supporting files are present, it also runs read-only line-ending, path-reference, Markdownlint, and optional PSScriptAnalyzer debt reports before fixers, then runs the placeholder scan, marker validation, and Markdown package scripts such as `npm run lint:md` and `npm run lint:md:links`. In `--fix` mode, the Markdown quality report becomes an explicit Markdown fixer command and reports changed files afterward. Intentional path-reference findings can be suppressed in the downstream-created `.template-sync/first-adoption/quality-suppressions.json` file.
